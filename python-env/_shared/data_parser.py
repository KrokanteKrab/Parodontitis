...
def calculate_age(birthdate, other_date=date.today()):
...
class DataParser:
    ...
    def convert_xml_to_dataframe(self, xml_string):
        ...
        for patient in root.iter('patient'):
            ...
            age = calculate_age(birthdate=birth_date_formatted)
            ...
            for medical_visit in medical_visits:
                ...
                visit_age = calculate_age(birthdate=birth_date_formatted, other_date=visit_date_formatted)
                ...
                # Has Parodontitis
                has_parodontitis_tag = medical_visit.find('hasParodontitis')
                has_parodontitis = 0
                if has_parodontitis_tag is not None:
                    has_parodontitis = int(has_parodontitis_tag.text)

                # Dental Diagnosis 'D4210' 'D4211'
                dental_diagnosis = medical_visit.find('dentalDiagnosis')
                dds_code_d4210 = 0
                dds_code_d4211 = 0
                if dental_diagnosis is not None:
                    match dental_diagnosis.find('DDSCode').text:
                        case 'D4210':
                            dds_code_d4210 = 1
                        case 'D4211':
                            dds_code_d4211 = 1
